...
@router.post("/stream/")
async def stream_mock_chat(request: Request):
    body = await request.json()
    _ = body.get("messages", [])  # unused for now
...
@router.post("/stream/lorem")
async def stream_lorem_ipsum(request: Request):
    body = await request.json()
    _ = body.get("messages", [])  # unused for now

    async def generate():
        for word in lorem.get_paragraph(
            count=1,
            comma=(0, 2),
            word_range=(4, 8),
            sentence_range=(5, 10),
